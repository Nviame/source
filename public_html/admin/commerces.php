...
<html lang="es">
    ...
    <body class="app">
                        ...
                        <div class="col-span-12 mt-8">
                            <div class="intro-y flex items-center h-10">
                                <h2 class="text-lg font-medium truncate mr-5">
                                    <?php
                                        $mainTitle = "Todos los comercios";
                                        switch($currentPHPScriptParam) {
                                            case "new":
                                                $mainTitle = "Comercios registrados recientemente";
                                            break;
                                            case "verified":
                                                $mainTitle = "Comercios verificados";
                                            break;
                                            case "disabled":
                                                $mainTitle = "Comercios inhabilitados";
                                            break;
                                        }
                                        echo $mainTitle;
                                    ?>
                                </h2>
                                <a href="javascript: CRUD.loadCommerces();" class="ml-auto flex text-theme-1"> <i data-feather="refresh-ccw" class="w-4 h-4 mr-3"></i> Refrescar datos </a>
                            </div>
                        </div>
                    </div>
                    <div class="intro-y col-span-12 flex flex-wrap sm:flex-no-wrap items-center mt-2">
                        <a data-toggle="modal" data-target="#md-user" class="button text-white bg-theme-1 shadow-md mr-2">Agregar comercio</a>
                        ...
                        <div class="hidden md:block mx-auto text-gray-600" id="table-commerces-header-info">Cargando comercios...</div>
                    </div>
                    <!-- BEGIN: Data List -->
                    <div class="intro-y datatable-wrapper box p-5 col-span-12">
                        <div id="table-commerces-wrapper">
                            <table id="table-commerces" class="table table-report table-report--bordered display w-full -mt-2">
                                <thead>
                                    <tr>
                                        <th class="whitespace-no-wrap"></th>
                                        <th class="whitespace-no-wrap">COMERCIO</th>
        ...
        <div class="modal" id="md-user">
            <form id="form-commerce" class="modal__content" style="width: 100%; max-width: 720px;">
                <div class="flex items-center px-5 py-5 sm:py-3 border-b border-gray-200">
                    <h2 class="font-medium text-base mr-auto">Agregar comercio</h2>
                </div>
        ...
        <script>
            ...
            function sendPush(i) {
                $('#md-push-content').html(`
                    <div class="col-span-12 sm:col-span-12">
                        <label class="mb-2" style="display: inline-flex;">Comercios</label>
                ...
                $('#table-commerces').DataTable().rows().every( function ( rowIdx, tableLoop, rowLoop ) {
                    var d = this.data();
                ...
                $('#md-push-content .select2').select2({
                    //allowClear: true,
                    closeOnSelect: false,
                    placeholder: 'Selecciona los comercios',
                    data: data
            ...
            function loadCompanies(selectedValue) {
                $('#form-commerce [name="company"]').html('<option value="0" selected>Cargando listado de empresas...</option>');
                API.get('companies', {}, (json) => {
                    if(!json.error) {
                        console.log(json.data);
                        $('#form-commerce [name="company"]').html('<option value="0" selected>Ninguna</option>');
                        json.data.forEach(e => {
                            $('#form-commerce [name="company"]').append(`<option value="${e.id}" data-commission="${e.percent_commission}">${e.name}</option>`);
                        });
                        if(typeof selectedValue != 'undefined') {
                            $('#form-commerce [name="company"]').val(selectedValue);
                        }
                    }
                });
            }
            function disU(i, a) {
                ...
                $('#disable-user-confirmation-modal-text').html(a == 1 ? 'Se bloqueará por completo el acceso de este comercio.' : 'Se desbloqueará por completo el acceso de este comercio.');
            ...
            function editU(i) {
                $('#form-commerce .validate-form,#md-user [type="submit"]').css({
                    opacity: 0.25,
                    pointerEvents: 'none'
                });
                $('#md-user [type="submit"]').addClass('disabled');
                $('#form-commerce').attr('data-mode', 'edit');
                $('#form-commerce').attr('data-target', i);
                $('#form-commerce .text-base').html('Modificar comercio');
                $('#md-user').modal('show');

                API.get(`users/${i}`, {}, (json) => {
                    var u = json.data;
                    loadCompanies(u.id_company || "0");
                    $('#form-commerce [name="name"]').val(u.display_name);
                    $('#form-commerce [name="dni"]').val(u.dni || "");
                    $('#form-commerce [name="phone"]').val(u.phone || "");
                    $('#form-commerce [name="email"]').val(u.email);
                    $('#form-commerce [name="company"]').val(u.id_company || "0");
                    $('#form-commerce [name="commission"]').val(u.commission || "");
                    $('#form-commerce [name="password"]').removeAttr("required");
                    $('#form-commerce .validate-form,#md-user [type="submit"]').css({
                        opacity: 1,
                        pointerEvents: 'auto'
                    });
            ...
            $('#form-commerce [name="company"]').change(function() {
                var value = this.value;
                var el = $(this).find(`option[value="${value}"]`);
                $('#form-commerce [name="commission"]').val(el.data('commission') || "");
            });
            const CRUD = {
                toggleVerification: (i) => {
                    function f() {
                        API.post(`users/${i}/toggle_verification`, {}, (json) => {
                            CRUD.loadCommerces();
                            $('#success-modal-title').html('Operación exitosa');
                            $('#success-modal-content').html('Se ha guardado el ajuste de verificación en el comercio.');
                            $('#success-modal').modal('show');
                        });
                    }
                    if($(`#table-commerces tr[data-uid="${i}"] .input--switch`).prop('checked')) {
                        f();
                    }
                    else {
                        ...
                        $('#question-modal [data-dismiss="modal"]').off('click').on('click', function() {
                            $(`#table-commerces tr[data-uid="${i}"] .input--switch`).prop('checked', true);
                        });
                        $('#question-modal').modal('show');
                    }
                },
                detailsUser: (i) => {
                    $('#md-details-user-title').html('Cargando información del comercio...');
                ...
                loadCommerces: () => {
                    $('#table-commerces-header-info').html('Cargando comercios...');
                    API.get('users', {
                        filter: '<?php echo $currentPHPScriptParam ? $currentPHPScriptParam : 'all' ?>',
                        type: 'commerce'
                    }, (json) => {
                        if(!json.error) {
                            $('#table-commerces_filter input[type="search"]').val("");
                            $('#table-commerces-wrapper').html(`
                                <table id="table-commerces" class="table table-report table-report--bordered display w-full -mt-2">
                                    <thead>
                                        <tr>
                                            <th class="whitespace-no-wrap"></th>
                            ...
                            json.data.forEach((info) => {
                                console.log(info);
                                const dir = info.pcommerce || "-";
                                $('#table-commerces tbody').append(`
                                    <tr>${info.id}</tr>
                                    <tr class="intro-x" data-uid="${info.id}">
                                        <td class="w-20">
                                            <div class="flex user-images" data-uid="${info.id}" data-u="${info.display_name}">
                                                <div class="w-10 h-10 image-fit zoom-in" style="min-width: 40px;">
                                                    ${info.dni_front ? `
                                                        <img alt="DNI o CUIT" class="tooltip rounded-full" src="${API.getCommercePicture(info.dni_front)}" title="DNI o CUIT">    
                                                    ` : ''}
                                                </div>
                                                <div class="w-10 h-10 image-fit zoom-in -ml-5">
                                                    ${info.logo ? `
                                                        <img alt="Foto" class="tooltip rounded-full" src="${API.getCommercePicture(info.logo)}" title="Foto de perfil">    
                                                    ` : ''}
                                                </div>
                                            </div>
                                        </td>
                                        ...
                                        <td class="table-report__action w-36">
                                            <div class="flex justify-center items-center">
                                                ...
                                                <a class="tooltip flex items-center text-theme-<?php echo $currentPHPScriptParam == "disabled" ? "1" : "6"; ?>" href="javascript:disU(${info.id}, <?php echo $currentPHPScriptParam == "disabled" ? 0 : 1; ?>);" data-toggle="modal" data-target="#disable-user-confirmation-modal" title="<?php echo $currentPHPScriptParam == "disabled" ? "Habilitar" : "Deshabilitar"; ?> comercio"> 
                                ...
                                $(`#table-commerces tbody tr[data-uid="${info.id}"] .user-images`).click(function() {
                            ...
                            feather.replace();
                            $('#table-commerces-header-info').html(json.data.length > 10 ? `${json.data.length} comercios en total.` : `${json.data.length} comercios en total.`);
                            
                            $('#table-commerces').addClass('datatable').DataTable().order( [ 0, 'desc' ] ).draw();
                        }
                    });
                }
            };
            ...
            $('#send-push').click(() => {
                ...
                if(u.length > 0) {
                    ...
                    error = "Selecciona al menos 1 comercio.";
                }
                if(error) {
            ...
            $(document).on('click', '[data-toggle="modal"]', (el) => {
                ...
                if(el.target.dataset.target == '#md-user') {
                    ...
                    $('#form-commerce [name]').val("");
                    $('#form-commerce [name="password"]').attr("required", "required");
                    $('#form-commerce').attr('data-mode', 'add');
                    $('#form-commerce .text-base').html('Agregar comercio');

                    loadCompanies();
                }
            });

            $("#form-commerce").on('submit', (ev) => {
                var fd = new FormData(document.getElementById("form-commerce"));
                var mode = $('#form-commerce').attr('data-mode');
                if(mode == 'edit') {
                    fd.append('i', $('#form-commerce').attr('data-target'));
                }
                $('#form-commerce [type="submit"]').css({
                    pointerEvents: 'none'
                }).animate({
                    opacity: 0.7
                }, 400);
                $.ajax({
                    ...
                    if(typeof data === 'object') {
                        if(data.status == "OK") {
                            ...
                            setTimeout(() => {
                                $('#success-modal-title').html('Operación exitosa');
                                $('#success-modal-content').html(`Se ha ${mode == 'add' ? 'guardado' : 'actualizado'} el comercio.`);
                                $('#success-modal').modal('show');
                                $('#form-commerce [type="submit"]').css({
                                    pointerEvents: 'auto'
                                }).animate({
