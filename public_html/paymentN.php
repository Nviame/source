<?php
    ...
    require_once __DIR__ . '/api/include/Config.php';
    require_once __DIR__ . '/api/include/DbHandler.php';
    require_once __DIR__ . '/api/vendor/autoload.php';

    define("DEFAULT_NV_FEE_PERCENT", 20);
    ...
    $pageRedirect = HOST . $_SERVER["REQUEST_URI"];
    ...
    if($shipmentInfo) {
        ...
        if($userInfo["info_commission"] && $userInfo["info_commission"] > 0) {
            $userComission = $userInfo["info_commission"];
        }
        else if($userInfo["info_commission_company"] && $userInfo["info_commission_company"] > 0) {
            $userComission = $userInfo["info_commission_company"];
        }
    }
    else {
        $offerInfo = null;
    ...
    $preference->notification_url = HOST . "/api/mp/ipn";

    $preference->marketplace_fee = 0;
    if($userComission > 0) {
        $preference->marketplace_fee = ($userComission * $offerInfo["offer"]) / 100;
    }
    else {
        $preference->marketplace_fee = (DEFAULT_NV_FEE_PERCENT * $offerInfo["offer"]) / 100;
    }
    ...
    $item->quantity = 1;
    $item->unit_price = floatval($offerInfo["offer"]);

    if(isset($_REQUEST["secop"])) {
        $item->unit_price += floatval($_REQUEST["secop"]);
    }
